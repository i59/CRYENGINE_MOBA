...
const char * GetTimeString( int secs, bool useShortForm/*=false*/, bool includeSeconds/*=true*/, bool useSingleLetters/*=false*/ )
{
    ...
    {
		  if (d==1)
			  result.Format("%d %s",d, g_pGame->Localize("@ui_day"));
		  else if (d>1)
			  result.Format("%d %s",d, g_pGame->Localize("@ui_days"));

		  if (h==1)
			  result.Format("%s%s%d %s",result.c_str(), result.empty()?"":" ", h, g_pGame->Localize("@ui_hr"));
		  else if (h>1 || d>0)
			  result.Format("%s%s%d %s",result.c_str(), result.empty()?"":" ", h, g_pGame->Localize("@ui_hrs"));

		  if (m==1)
			  result.Format("%s%s%d %s",result.c_str(), result.empty()?"":" ", m, g_pGame->Localize("@ui_min"));
		  else if (m>1 || h>0 || d>0)
			  result.Format("%s%s%d %s",result.c_str(), result.empty()?"":" ", m, g_pGame->Localize("@ui_mins"));

		  if(includeSeconds)
		  {
			  if (s==1)
				  result.Format("%s%s%d %s",result.c_str(), result.empty()?"":" ", s, g_pGame->Localize("@ui_sec"));
			  else
				  result.Format("%s%s%d %s",result.c_str(), result.empty()?"":" ", s, g_pGame->Localize("@ui_secs"));
		  }
    }
    else
    {
      if (d>0)
        result.Format("%d %s",d, g_pGame->Localize("@ui_mp_days"));

      if (h>0 || d>0)
        result.Format("%s%s%d %s",result.c_str(), result.empty()?"":" ", h, g_pGame->Localize("@ui_mp_hrs"));

      if (m>0 || h>0 || d>0)
        result.Format("%s%s%d %s",result.c_str(), result.empty()?"":" ", m, g_pGame->Localize("@ui_mp_mins"));

      if(includeSeconds)
      {
        if (s>0 || (h==0 && d==0 && m==0))
          result.Format("%s%s%d %s",result.c_str(), result.empty()?"":" ", s, g_pGame->Localize("@ui_mp_sec"));
      }
    }
	}
...
const wchar_t * GetTimeStringW( int secs, bool useShortForm/*=false*/, bool includeSeconds/*=true*/, bool useSingleLetters/*=false*/)
{
	...
	else
	{
		...
		{
			if (d==1)
				resultW.Format(L"%d %ls",d, g_pGame->Localize("@ui_day"));
			else if (d>1)
				resultW.Format(L"%d %ls",d, g_pGame->Localize("@ui_days"));

			if (h==1)
				resultW.Format(L"%ls%ls%d %ls",resultW.c_str(), resultW.empty()?L"":L" ", h, g_pGame->Localize("@ui_hr"));
			else if (h>1 || d>0)
				resultW.Format(L"%ls%ls%d %ls",resultW.c_str(), resultW.empty()?L"":L" ", h, g_pGame->Localize("@ui_hrs"));

			if (m==1)
				resultW.Format(L"%ls%ls%d %ls",resultW.c_str(), resultW.empty()?L"":L" ", m, g_pGame->Localize("@ui_min"));
			else if (m>1 || h>0 || d>0)
				resultW.Format(L"%ls%ls%d %ls",resultW.c_str(), resultW.empty()?L"":L" ", m, g_pGame->Localize("@ui_mins"));

			if(includeSeconds)
			{
				if (s==1)
					resultW.Format(L"%ls%ls%d %ls",resultW.c_str(), resultW.empty()?L"":L" ", s, g_pGame->Localize("@ui_sec"));
				else
					resultW.Format(L"%ls%ls%d %ls",resultW.c_str(), resultW.empty()?L"":L" ", s, g_pGame->Localize("@ui_secs"));
			}
		}
		else
		{
			if (d>0)
				resultW.Format(L"%d %ls",d, g_pGame->Localize("@ui_mp_days"));

			if (h>0 || d>0)
				resultW.Format(L"%ls%ls%d %ls",resultW.c_str(), resultW.empty()?L"":L" ", h, g_pGame->Localize("@ui_mp_hrs"));

			if (m>0 || h>0 || d>0)
				resultW.Format(L"%ls%ls%d %ls",resultW.c_str(), resultW.empty()?L"":L" ", m, g_pGame->Localize("@ui_mp_mins"));

			if(includeSeconds)
			{
				if (s>0 || (h==0 && d==0 && m==0))
					resultW.Format(L"%ls%ls%d %ls",resultW.c_str(), resultW.empty()?L"":L" ", s, g_pGame->Localize("@ui_mp_sec"));
			}
		}
	}
